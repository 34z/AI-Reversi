...
def get_dis_score(board, color):
    ...
    dis_this = np.sum((cordinates_this - avg_this) ** 2) / cordinates_this.shape[0]
    dis_that = np.sum((cordinates_that - avg_that) ** 2) / cordinates_that.shape[0]
    return sigmoid(dis_that - dis_this)
...
def get_action_score(board, color):
    ...
    return sigmoid(len(actions_this)-len(actions_that))
...
def get_stable_score(board, color):
    ...
    return sigmoid(stable_this_cnt - stable_that_cnt)
...
class AIPlayer:
    ...
    def _alpha_beta_prunig(self, board, color, step, alpha, beta):
        ...
        if step > 0:
            ...
            if self.score == 0:
                ...
                val = get_win_score(board, self.color)
        return action, val
    ...
    def get_score(self, board):
        ...
        scores = np.array([self.get_dis_score(board), self.get_action_score(board), self.get_stable_score(board)])
        if cnt < 20:
